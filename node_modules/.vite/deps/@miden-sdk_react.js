...
// node_modules/@miden-sdk/react/dist/index.mjs
...
var SignerContext = (0, import_react3.createContext)(null);
function useSigner() {
  return (0, import_react3.useContext)(SignerContext);
}
...
function MidenProvider({
  ...
  if (isInitializing && loadingComponent) {
    return (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: loadingComponent });
  }
  if (initError && errorComponent) {
    if (typeof errorComponent === "function") {
      return (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: errorComponent(initError) });
    }
    return (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: errorComponent });
  }
  const contextValue = {
    client,
    isReady,
  ...
  return (0, import_jsx_runtime.jsx)(MidenContext.Provider, { value: contextValue, children });
}
function useMiden() {
...
var MultiSignerRegistryContext = (0, import_react4.createContext)(null);
var MultiSignerContext = (0, import_react4.createContext)(null);
function MultiSignerProvider({ children }) {
  const signersRef = (0, import_react4.useRef)(/* @__PURE__ */ new Map());
  const [signersSnapshot, setSignersSnapshot] = (0, import_react4.useState)(
    []
  );
  const [activeSignerName, setActiveSignerName] = (0, import_react4.useState)(null);
  const activeSignerNameRef = (0, import_react4.useRef)(null);
  const generationRef = (0, import_react4.useRef)(0);
  const updateSnapshot = (0, import_react4.useCallback)(() => {
  ...
  const stableConnect = (0, import_react4.useCallback)(async () => {
    const name = activeSignerName;
    if (!name) throw new Error("No active signer (signer was disconnected)");
    const signer = signersRef.current.get(name);
    if (!signer) throw new Error(`Signer "${name}" not found in registry`);
  ...
  const stableDisconnect = (0, import_react4.useCallback)(async () => {
    const name = activeSignerName;
    if (!name) throw new Error("No active signer (signer was disconnected)");
    const signer = signersRef.current.get(name);
    if (!signer) throw new Error(`Signer "${name}" not found in registry`);
  ...
  const forwardedValue = (0, import_react4.useMemo)(() => {
  ...
  const setActiveName = (0, import_react4.useCallback)((name) => {
    activeSignerNameRef.current = name;
    setActiveSignerName(name);
  }, []);
  const connectSigner = (0, import_react4.useCallback)(
    async (name) => {
  ...
  const disconnectSigner = (0, import_react4.useCallback)(async () => {
  ...
  const multiSignerValue = (0, import_react4.useMemo)(
    () => ({
  ...
  return (0, import_jsx_runtime2.jsx)(SignerContext.Provider, { value: forwardedValue, children: (0, import_jsx_runtime2.jsx)(MultiSignerRegistryContext.Provider, { value: registry, children: (0, import_jsx_runtime2.jsx)(MultiSignerContext.Provider, { value: multiSignerValue, children }) }) });
}
function SignerSlot() {
  const signerValue = useSigner();
  const registry = (0, import_react4.useContext)(MultiSignerRegistryContext);
  const nameRef = (0, import_react4.useRef)();
...
function useMultiSigner() {
  return (0, import_react4.useContext)(MultiSignerContext);
}
...
function useAssetMetadata(assetIds = []) {
  ...
  const rpcClient = (0, import_react7.useMemo)(() => getRpcClient(rpcUrl), [rpcUrl]);
  const uniqueAssetIds = (0, import_react7.useMemo)(
...
function useNotes(options) {
  ...
  const noteAssetIds = (0, import_react8.useMemo)(() => {
  ...
  const getMetadata = (0, import_react8.useCallback)(
    (assetId) => assetMetadata.get(assetId),
    [assetMetadata]
  );
  const normalizedSender = (0, import_react8.useMemo)(() => {
  ...
  const filterBySender = (0, import_react8.useCallback)(
    (summaries, target) => {
  ...
  const noteSummaries = (0, import_react8.useMemo)(() => {
  ...
  const consumableNoteSummaries = (0, import_react8.useMemo)(() => {
...
function useNoteStream(options = {}) {
  ...
  const handledIdsRef = (0, import_react9.useRef)(/* @__PURE__ */ new Set());
  const [handledVersion, setHandledVersion] = (0, import_react9.useState)(0);
  const status = options.status ?? "committed";
  const sender = options.sender ?? null;
  const since = options.since;
  const amountFilterRef = (0, import_react9.useRef)(options.amountFilter);
  amountFilterRef.current = options.amountFilter;
  const excludeIdsKey = (0, import_react9.useMemo)(() => {
    if (!options.excludeIds) return "";
    if (options.excludeIds instanceof Set)
      return Array.from(options.excludeIds).sort().join("\0");
    return [...options.excludeIds].sort().join("\0");
  }, [options.excludeIds]);
  const excludeIdSet = (0, import_react9.useMemo)(() => {
  ...
  const streamedNotes = (0, import_react9.useMemo)(() => {
    void handledVersion;
    const result = [];
  ...
  const snapshot = (0, import_react9.useCallback)(() => {
    const ids = /* @__PURE__ */ new Set();
    for (const note of streamedNotes) {
      ids.add(note.id);
    }
...
function useTransactionHistory(options = {}) {
  const { client, isReady } = useMiden();
  const { lastSyncTime } = useSyncStateStore();
  const [records, setRecords] = (0, import_react10.useState)([]);
  const [isLoading, setIsLoading] = (0, import_react10.useState)(false);
  const [error, setError] = (0, import_react10.useState)(null);
  const rawIds = (0, import_react10.useMemo)(() => {
    if (options.id) return [options.id];
    if (options.ids && options.ids.length > 0) return options.ids;
    return null;
  }, [options.id, options.ids]);
  const idsHex = (0, import_react10.useMemo)(() => {
...
function useSyncState() {
  ...
  const sync = (0, import_react11.useCallback)(async () => {
    await triggerSync();
  }, [triggerSync]);
  return {
    ...syncState,
...
function useCreateWallet() {
  ...
  const createWallet = (0, import_react12.useCallback)(
    async (options = {}) => {
      if (!client || !isReady) {
...
function useCreateFaucet() {
  ...
  const runExclusiveSafe = runExclusive ?? runExclusiveDirect;
  const setAccounts = useMidenStore((state) => state.setAccounts);
  const [faucet, setFaucet] = (0, import_react13.useState)(null);
  const [isCreating, setIsCreating] = (0, import_react13.useState)(false);
  const [error, setError] = (0, import_react13.useState)(null);
  const createFaucet = (0, import_react13.useCallback)(
    async (options) => {
      if (!client || !isReady) {
        throw new Error("Miden client is not ready");
      }
...
function useImportAccount() {
  ...
  const setAccounts = useMidenStore((state) => state.setAccounts);
  const [account, setAccount] = (0, import_react14.useState)(null);
  const [isImporting, setIsImporting] = (0, import_react14.useState)(false);
  const [error, setError] = (0, import_react14.useState)(null);
  const importAccount = (0, import_react14.useCallback)(
    async (options) => {
      if (!client || !isReady) {
        throw new Error("Miden client is not ready");
      }
...
function useMultiSend() {
  ...
  const isBusyRef = (0, import_react16.useRef)(false);
  ...
  const sendMany = (0, import_react16.useCallback)(
    async (options) => {
      if (!client || !isReady) {
        throw new Error("Miden client is not ready");
      }
...
function useWaitForCommit() {
  const { client, isReady } = useMiden();
  const waitForCommit = (0, import_react17.useCallback)(
    async (txId, options) => {
      if (!client || !isReady) {
        throw new Error("Miden client is not ready");
      }
...
function useWaitForNotes() {
  const { client, isReady, runExclusive } = useMiden();
  const runExclusiveSafe = runExclusive ?? runExclusiveDirect;
  const waitForConsumableNotes = (0, import_react18.useCallback)(
    async (options) => {
      if (!client || !isReady) {
        throw new Error("Miden client is not ready");
      }
...
function useMint() {
  ...
  const mint = (0, import_react19.useCallback)(
    async (options) => {
      if (!client || !isReady) {
        throw new Error("Miden client is not ready");
      }
...
function useConsume() {
  ...
  const consume = (0, import_react20.useCallback)(
    async (options) => {
      if (!client || !isReady) {
        throw new Error("Miden client is not ready");
      }
...
function useTransaction() {
  ...
  const [stage, setStage] = (0, import_react22.useState)("idle");
...
function useExecuteProgram() {
  ...
  const execute = (0, import_react23.useCallback)(
    async (options) => {
      if (!client || !isReady) {
        throw new Error("Miden client is not ready");
      }
...
function useCompile() {
  const { client, isReady } = useMiden();
  const resource = (0, import_react24.useMemo)(
    () => client && isReady ? new CompilerResource(client, getWasmOrThrow) : null,
    [client, isReady]
  );
  const requireResource = (0, import_react24.useCallback)(() => {
    if (!resource) {
      throw new Error("Miden client is not ready");
    }
    return resource;
  }, [resource]);
  const component = (0, import_react24.useCallback)(
    async (options) => requireResource().component(options),
    [requireResource]
  );
  const txScript = (0, import_react24.useCallback)(
    async (options) => requireResource().txScript(options),
    [requireResource]
  );
  const noteScript = (0, import_react24.useCallback)(
...
function useSessionAccount(options) {
  ...
  const setAccounts = useMidenStore((state) => state.setAccounts);
  const [sessionAccountId, setSessionAccountId] = (0, import_react25.useState)(null);
  const [step, setStep] = (0, import_react25.useState)("idle");
  const [error, setError] = (0, import_react25.useState)(null);
  const cancelledRef = (0, import_react25.useRef)(false);
  const isBusyRef = (0, import_react25.useRef)(false);
  ...
  const fundRef = (0, import_react25.useRef)(options.fund);
  ...
  const initialize = (0, import_react25.useCallback)(async () => {
    if (!client || !isReady) {
      throw new Error("Miden client is not ready");
    }
    if (isBusyRef.current) {
...
function useExportStore() {
  const { client, isReady, runExclusive } = useMiden();
  const [isExporting, setIsExporting] = (0, import_react26.useState)(false);
  const [error, setError] = (0, import_react26.useState)(null);
  const exportStore = (0, import_react26.useCallback)(async () => {
    if (!client || !isReady) {
      throw new Error("Miden client is not ready");
    }
    setIsExporting(true);
...
function useImportStore() {
  ...
  const importStore = (0, import_react27.useCallback)(
...
function useImportNote() {
  ...
  const importNote = (0, import_react28.useCallback)(
    async (noteBytes) => {
      if (!client || !isReady) {
        throw new Error("Miden client is not ready");
      }
...
function useExportNote() {
  ...
  const exportNote = (0, import_react29.useCallback)(
    async (noteId) => {
      if (!client || !isReady) {
        throw new Error("Miden client is not ready");
      }
...
function useSyncControl() {
  ...
  const pauseSync = (0, import_react30.useCallback)(() => setSyncPaused(true), [setSyncPaused]);
  const resumeSync = (0, import_react30.useCallback)(() => setSyncPaused(false), [setSyncPaused]);
  return {
    pauseSync,
    resumeSync,
    isPaused
...
//# sourceMappingURL=@miden-sdk_react.js.map
