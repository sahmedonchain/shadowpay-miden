 ...
 * ```
 */
export const MidenFiSignerProvider = ({ children, wallets: walletsProp, appName = 'Miden DApp', network = WalletAdapterNetwork.Testnet, autoConnect = false, privateDataPermission = PrivateDataPermission.UponRequest, allowedPrivateData = AllowedPrivateData.None, onError, localStorageKey = 'walletName', accountType = 'RegularAccountImmutableCode', storageMode = 'public', customComponents, importAccountId, }) => {
    ...
    const requestConsume = useMemo(() => adapter && 'requestConsume' in adapter
        ...
        : undefined, [adapter, handleError, connected]);
    const createAccount = useMemo(() => adapter && 'createAccount' in adapter
        ? async (params) => {
            if (!connected)
                throw handleError(new WalletNotConnectedError());
